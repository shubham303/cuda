...
__global__ void dkernel_Mins(int *gpuOA, int *gpuCA, int *gpulocals,
	...
	if (id <= size) {
		int minimum = gpulocals[id - 1];
		int i = gpuOA[id - 1];
		int j = gpuOA[id];
		while (i < j) {
			minimum = min(minimum, gpucurrentupdate[gpuCA[i]]);
			i++;
		}
		gpulocals[id - 1] = minimum;
	}
...
__global__ void dkernel_Maxs(int *gpuOA, int *gpuCA, int *gpulocals,
	...
	int id = blockDim.x * blockIdx.x + threadIdx.x + 1;
	if (id <= size) {
		int maximum = gpulocals[id - 1];

		int i = gpuOA[id - 1];
		int j = gpuOA[id];
		while (i < j) {
			maximum = max(maximum, gpucurrentupdate[gpuCA[i]]);
			i++;
		}
		gpulocals[id - 1] = maximum;
	}
}
